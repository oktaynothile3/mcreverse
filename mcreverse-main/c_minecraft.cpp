...
void c_main::hook( void ) {
	...
	while ( !( minecraft->m_jvm_dll = GetModuleHandleA( XOR_OP ( "jvm.dll" ) ) ) ) {
		std::this_thread::sleep_for( std::chrono::milliseconds( 500 ) );
	}

	while ( !( minecraft->m_opengl_dll = GetModuleHandleA( XOR_OP ( "opengl32.dll" ) ) ) ) {
		std::this_thread::sleep_for( std::chrono::milliseconds( 500 ) );
	}

	while ( !( minecraft->m_kernel32_dll = GetModuleHandleA( XOR_OP ( "Kernel32.dll" ) ) ) ) {
		std::this_thread::sleep_for( std::chrono::milliseconds( 500 ) );
	}
	...
	while ( !( get_window_title( ).find( XOR_OP ( "Minecraft" ) ) != std::string::npos ) ) {
		std::this_thread::sleep_for( std::chrono::milliseconds( 500 ) );
	}
	...
	minecraft->m_wgl_swap_buffers = GetProcAddress( minecraft->m_opengl_dll, XOR_OP ( "wglSwapBuffers" ) );
	minecraft->m_open_process = GetProcAddress( minecraft->m_kernel32_dll, XOR_OP ( "OpenProcess" ) );
	minecraft->m_create_remote_thread = GetProcAddress( minecraft->m_kernel32_dll, XOR_OP ( "CreateRemoteThread" ) );
	minecraft->m_enum_process_modules = GetProcAddress( minecraft->m_kernel32_dll, XOR_OP ( "EnumProcessModules" ) );
